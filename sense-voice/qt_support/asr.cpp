...
ASRServer::asr_params::asr_params():
    ...
    max_len(0),
    n_mel(80),
    best_of(sense_voice_full_default_params(SENSE_VOICE_SAMPLING_GREEDY).greedy.best_of),
    beam_size(sense_voice_full_default_params(SENSE_VOICE_SAMPLING_BEAM_SEARCH).beam_search.beam_size),
    audio_ctx(0),
    word_thold(0.01f),
    entropy_thold(2.40f),
    logprob_thold(-1.00f),
...
ASRServer::asr_result::asr_result():
    request_id(INVALID_ID),
    text() {}

ASRServer::asr_result::asr_result(task_id_t id, std::string txt):
    request_id(id),
    text(txt) {}
...
const char *ASRServer::print_system_info() {
    static std::string s;
    s  = "";
...
void ASRServer::print_usage(int /*argc*/, char **argv, const asr_params &params) {
    fprintf(stderr, "\n");
...
char *ASRServer::param_turn_lowercase(char *in) {
...
bool ASRServer::asr_params_parse(int argc, char **argv, asr_params &params) {
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            ASRServer::print_usage(argc, argv, params);
            exit(0);
        }
        ...
        else if (arg == "-l"    || arg == "--language")        { params.language        = ASRServer::param_turn_lowercase(argv[++i]); }
        ...
        else if (arg == "-pr"   || arg == "--need-prefix")     { params.need_prefix     = true; }
...
// implementation of SENSEVOICE_API
...
int ASRServer::cli_main(int argc, char **argv) {
    asr_params params;

    if (!ASRServer::asr_params_parse(argc, argv, params)) {
        ASRServer::print_usage(argc, argv, params);
        return 1;
    }
    ...
    for (int f = 0; f < (int) params.fname_inp.size(); ++f) {
        ...
        if (!params.no_prints) {
            ...
            fprintf(stderr, "system_info: n_threads = %d / %d | %s\n",
                    params.n_threads*params.n_processors, std::thread::hardware_concurrency(), ASRServer::print_system_info());
...
ASRServer::asr_result ASRServer::handle(
    ...
    if (request_id < 0) return {INVALID_ID, ""};
    auto info = [_info](const char *buf) { if (_info) _info(buf); };
    auto error = [_error](const char *buf) { if (_error) _error(buf); };
    char logbuf[LOG_LINE_BUFSIZE];
    std::string tmp_resline;
    std::ostringstream result;
    this->currentId = request_id;
    this->currentResult.request_id = request_id;
    this->currentResult.text = "";

    if (!is_file_exist(params.model.c_str())) {
        snprintf(logbuf, LOG_LINE_BUFSIZE, "model not found '%s'", params.model.c_str());
        error(logbuf);
        return {INVALID_ID, ""};
    }
    ...
    for (auto it = params.fname_inp.begin(); it != params.fname_inp.end();) {
        ...
        if (*it != "-" && !is_file_exist(fname_inp)) {
            snprintf(logbuf, LOG_LINE_BUFSIZE, "input file not found '%s'", fname_inp);
            error(logbuf);
            it = params.fname_inp.erase(it);
            continue;
        }
    ...
    if (params.language != "auto" && sense_voice_lang_id(params.language.c_str()) == -1) {
        snprintf(logbuf, LOG_LINE_BUFSIZE, "unknown language '%s'", params.language.c_str());
        error(logbuf);
        return {INVALID_ID, ""};
    }
    ...
    if (ctx == nullptr) {
        error("failed to initialize sense voice context");
        return {INVALID_ID, ""};
    }
    ...
    for (int f = 0; f < (int) params.fname_inp.size(); ++f) {
        ...
        int sample_rate;
        if (!::load_wav_file(fname_inp.c_str(), &sample_rate, pcmf32)) {
            snprintf(logbuf, LOG_LINE_BUFSIZE, "failed to read WAV file '%s'", fname_inp.c_str());
            error(logbuf);
            continue;
        }
        ...
        if (!params.no_prints) {
            // print system information
            info("\n");
            snprintf(logbuf, LOG_LINE_BUFSIZE, "system_info: n_threads = %d / %d | %s",
                    params.n_threads*params.n_processors, std::thread::hardware_concurrency(), ASRServer::print_system_info());
            info(logbuf);

            // print some info about the processing
            snprintf(logbuf, LOG_LINE_BUFSIZE, "%s: processing audio (%d samples, %.5f sec) , %d threads, %d processors, lang = %s...",
                    __func__,  int(pcmf32.size()), float(pcmf32.size())/sample_rate,
                    params.n_threads, params.n_processors,
                    params.language.c_str());
            info(logbuf);
...
//          ggml_backend_sched_set_eval_callback(sched,  ctx->params.cb_eval, &ctx->params.cb_eval_user_data);
                    ...
                    if (speech_prob < params.neg_threshold && triggered){
                        ...
                        if (i - temp_end < min_silence_samples) {
                            ...
                            if (current_speech_end - current_speech_start > min_speech_samples) {
                                ...
                                speech_segment.clear();
                                speech_segment.assign(pcmf32.begin() + current_speech_start, pcmf32.begin() + current_speech_end);
                                snprintf(logbuf, LOG_LINE_BUFSIZE, "[%.2f-%.2f] ", current_speech_start / (sample_rate * 1.0), current_speech_end / (sample_rate * 1.0));
                                info(logbuf);
                                if (sense_voice_full_parallel(ctx, wparams, speech_segment, speech_segment.size(), params.n_processors) != 0) {
                                    error("failed to process audio");
                                    return {INVALID_ID, ""};
        ...
        snprintf(logbuf, LOG_LINE_BUFSIZE, "%s: decoder audio use %f s, rtf is %f. \n",
                              __func__,
                              (ctx->state->t_encode_us + ctx->state->t_decode_us) / 1e6,
                              (ctx->state->t_encode_us + ctx->state->t_decode_us) / (1e6 * ctx->state->duration));
        info(logbuf);
