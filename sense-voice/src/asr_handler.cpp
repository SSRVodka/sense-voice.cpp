...
ASRHandler::asr_result::asr_result():
    request_id(INVALID_ID),
    text() {}

ASRHandler::asr_result::asr_result(task_id_t id, std::string txt):
    request_id(id),
    text(txt) {}
...
void ASRHandler::print_usage(int /*argc*/, char **argv, const asr_params &params) {
    fprintf(stderr, "\n");
...
char *ASRHandler::param_turn_lowercase(char *in) {
...
bool ASRHandler::asr_params_parse(int argc, char **argv, asr_params &params) {
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            ASRHandler::print_usage(argc, argv, params);
            exit(0);
        }
        ...
        else if (arg == "-l"    || arg == "--language")        { params.language        = ASRHandler::param_turn_lowercase(argv[++i]); }
...
int ASRHandler::cli_main(int argc, char **argv) {
    asr_params params;

    if (!ASRHandler::asr_params_parse(argc, argv, params)) {
        ASRHandler::print_usage(argc, argv, params);
        return 1;
    }
    ...
    for (int f = 0; f < (int) params.fname_inp.size(); ++f) {
        ...
        if (!params.no_prints) {
            ...
            fprintf(stderr, "system_info: n_threads = %d / %d | %s\n",
                    params.n_threads*params.n_processors, std::thread::hardware_concurrency(), ASRHandler::print_system_info());
...
ASRHandler::asr_result ASRHandler::handle(
