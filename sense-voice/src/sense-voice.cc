...
std::string sense_voice_print_output(struct sense_voice_context * ctx, bool need_prefix, bool use_itn, bool refresh_self) {
    ...
    // Start index based on the need_prefix flag
    int start_index = need_prefix ? 0 : 4;
    int prev_id = -1;

    for (int i = start_index; i < ids.size(); ++i) {
        int id = ids[i];
        // Skip if the current id is the same as the previous one
        if (id == prev_id) {
            continue;
        }
        // Add token if id is non-zero
        if (id != 0) {
            buf << ctx->vocab.id_to_token[id];
        }
        // Update prev_id to the current id
        prev_id = id;
    }
    // Optionally append a newline if refresh_self is false
    if (!refresh_self) {
        buf << '\n';
    }
